...
void scanline_convert( struct matrix *polygons, int i, screen s, color c, zbuffer zb ) {
 
  int y1 = polygons->m[1][i];
  int y2 = polygons->m[1][i+1];
  int y3 = polygons->m[1][i+2];

  //identify the order of the polygons, y-wise.
  int min, mid, max;
  ...
  float xT = polygons->m[0][maxI];
  float yT = polygons->m[1][maxI];
  float zT = polygons->m[2][maxI];
  float xM = polygons->m[0][midI];
  float yM = polygons->m[1][midI];
  float zM = polygons->m[2][midI];
  float xL = polygons->m[0][minI];
  float yL = polygons->m[1][minI];
  float zL = polygons->m[2][minI];
  
  float x0 = polygons->m[0][minI];
  float x1 = polygons->m[0][minI];
  float scany = yL;
  float z0 = polygons->m[2][minI];
  float z1 = polygons->m[2][minI];

  float startX = xL; 
  float endX = xL;
  float startZ;
  float endZ;
  float dx0 = yT == yL ? 0 : (xT - xL) * 1.0 /(yT - yL); 
  float dx1 = yM == yL ? 0 : (xM - xL) * 1.0 /(yM - yL); 
  float dx2 = yT == yM ? 0 : (xT - xM) * 1.0 /(yT - yM);
  float dz0 = yT == yL ? 0 : (zT - zL) * 1.0 /(yT - yL); 
  float dz1 = yM == yL ? 0 : (zM - zL) * 1.0 /(yM - yL); 
  float dz2 = yT == yM ? 0 : (zT - zM) * 1.0 /(yT - yM);
  ...
  while ( scany <= yM ) {
    ...
    c.blue = (i * 17) % 256;
    c.red = (i * 23) % 256;
    draw_line((int)startX, (int)scany, startZ, (int)endX, (int)scany, endZ, s, zb, c); // replace w/ real z values
    startX += dx0;
    endX += dx1;
    startZ += dz0;
    endZ += dz1;
  ...
  //calibrate
  x1 = polygons->m[0][midI];
  z1 = polygons->m[2][midI];
  ...
  while ( scany < yT ) {
    ...
    c.blue = (i * 17) % 256;
    c.red = (i * 23) % 256;
    draw_line((int)startX, (int)scany, startZ, (int)endX, (int)scany, endZ, s, zb, c);
    startX += dx0;
    endX += dx2;
    startZ += dz0;
    endZ += dz2;
  ...
  /*======== void draw_polygons() ==========
    ...
    color c  
    Returns: 
    Goes through polygons 3 polygons at a time, drawing 
